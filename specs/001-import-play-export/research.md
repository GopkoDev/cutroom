...
the renderer's `OffscreenCanvas`, advanced Frame by Frame with `add(frame / fps, 1 / fps)`. Audio
...
`secondsToFrame`, `frameToSampleIndex`, and the rational frame rate itself. Every call into
Mediabunny or Web Audio passes through it. Lint rule or review check: no arithmetic on frame rate
outside that module.
...
deserves fast tests. Fixtures are small generated files committed to the repo, including one with
a frame rate that differs from the Project's Timebase (edge case in the spec) and one with no
audio track.
